...
void Lander::draw( mat4 &worldToViewTransform )
{
  ...
  glBindVertexArray( VAO );
	...
	mat4 move = translate( position.x, position.y, 0);
	mat4 revolve = rotate( orientation, vec3(0,0,1) );
	
	mat4 correctLocationAndOrientation = worldToViewTransform * move * revolve ;
	
	glUniformMatrix4fv(glGetUniformLocation( myGPUProgram->id(), "MVP"), 1, GL_TRUE, &correctLocationAndOrientation[0][0]);
